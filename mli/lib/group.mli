...
theory G. -- Group.
  ...
  formal system. 
    function *. -- binary infix.
  
    object x, y, z.

  axiom G1. (x * y) * z = x * (y * z).
  axion G2. ∃ x: (∀ y x * y = y) ∧ ∀ y ∃ z z * y = x.
  
  end formal system.

  constant e,
  function ^-1. -- unary postfix.
  
definition g1. P(x) ≔ (∀ y x * y = y) ∧ ∀ y ∃ z z * y = x.

lemma g2. ∀ u, v: P(u) ∧ P(v) ⇒ u = v.
proof.
  claim a. P(u), P(v) ⊢ u = v.
  proof
    ...
    ∎
    a7. ∃ z z * v = u ⊢ u = v by ... -- existence introduction.
    conclusion by a7, a6.
  ∎
  conclusion by a, DTx.
∎
...
proof.
  1. ∃! P(x) by G2, g1, g2.
  2. conclusion by ... -- Defining axiom theorem.
∎ 

lemma g3. x * y = x * z ⇒ y = z.
proof.
  1. ∃ x' ∀ y x' * y = e by unit.
  claim a. x' * y = e, x * y = x * z ⊢ y = z.
  proof.
